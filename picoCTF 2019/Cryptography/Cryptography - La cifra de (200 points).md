...
After a connection to `2019shell1.picoctf.com:37608`, we receive what seems to be encrypted using Vigenere cipher:

```bash
root@kali:~/Downloads# nc 2019shell1.picoctf.com 37608
Encrypted message:
...
It can be easily brute-forced using [online tools](https://www.dcode.fr/chiffre-vigenere). What is interesting here, is that this message is a concatenation of several messages encrypted using Vigener cipher with different keys (anagrams of FLAG):
...
Yse Bnretèwp Cousex nd tnjceltce ytxeznxey hllrjo tnj Llhjcto Itsi tc Argprzn Nivmpr.
The Vigenere Cipher is therefore sometimes called the Alberti Disc or Alberti Cipher.

Os 1508, Uonfynkx Eroysesnfs osgetypd zmp su-hllrjo tggflg wpczf (l mgycid tq snnqtki llvmlbkyd) tnfe wuzwd rfeex gp a iwttohll itxpuspnz tq tnj Gimjyèrk Htpnjc.
In 1508, Johannes Trithemius invented the so-called tabula recta (a matrix of shifted alphabets) that would later be a critical component of the Vigenere cipher.
